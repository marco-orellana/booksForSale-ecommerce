...
public class PanierM {
    ...
    public static void add(HttpServletRequest request, Livre livreToAdd) {
        ...
        HashMap<Integer, LivrePanierWrapper> panier = (HashMap) SessionM.get(request, "panier");

        if (panier == null) {
            panier = PanierM.createPanier(request);
        }

        LivrePanierWrapper lpw = null;
        int idLivre = livreToAdd.getId();

        if (panier.get(idLivre) != null) {
    ...
    public static HashMap<Integer, LivrePanierWrapper> createPanier(HttpServletRequest request) {
